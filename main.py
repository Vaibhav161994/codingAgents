...
async def root():
    ...
    return FileResponse("static_site/app_team.html")
...
async def chat(websocket: WebSocket):
    ...
    # Send a properly formatted starter prompt to the user.
    starter_prompt = {
        "source": "system",
        "type": "message",  # Add a type field if your frontend expects it.
        "content": markdown(Path('prompts/initial_system_prompt.txt').read_text().strip()),  # Ensure no extra whitespace.
        "isHtml": True
    }
    await websocket.send_json(starter_prompt)
                ...
                async for message in stream:
                   ...
                   # Format message content with markdown
                    ...
                    if 'content' in message_dict:
                        message_dict['content'] = markdown(message_dict['content'])
                        message_dict['isHtml'] = True
